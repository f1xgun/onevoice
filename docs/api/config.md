...
| `SelfHostedEndpoints` | `SELF_HOSTED_N_URL` / `_MODEL` / `_API_KEY` | (empty) | `parseIndexedEndpoints` scans `N = 0, 1, 2, …` stopping when `SELF_HOSTED_N_URL` is missing. Entries without `MODEL` are skipped. Lifted verbatim from `services/orchestrator/internal/config/config.go` so byte-identical semantics apply on the API side. Every self-hosted request carries `x-data-logging-enabled: false` so a Yandex AI Studio endpoint does not retain the prompt payload. |
| `AllowTransborderLLM` | `ALLOW_TRANSBORDER_LLM` | `false` | Production LLM residency gate (152-ФЗ). With `APP_ENV=production` and this flag `false`, `Load` refuses to boot when any hosted key (`OPENROUTER_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY`) is set or when `LLM_MODEL` / `TITLER_MODEL` is not served by a `SELF_HOSTED_N_*` endpoint (`llm.EnforceResidency`). Set `true` only with a filed cross-border legal basis. **Boot error** on a non-boolean value. Mirrors the orchestrator flag. |
