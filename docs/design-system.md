...
## Проекты в списке чатов и переходы из задач

На `/chat` доступны отдельные действия «Новый проект» и «Новый чат» с учётом
прав пользователя. Проекты без чатов остаются в списке и предлагают создать
первый чат внутри проекта; поиск учитывает названия таких проектов.

Кнопка «Открыть чат» в задаче ведёт только в подтверждённый исходный диалог,
доступный текущему пользователю. Задачи синхронизации профиля ведут к профилю
организации. При отсутствии достоверной связи с диалогом подпись и пояснение
должны явно описывать доступный переход; нельзя подбирать чат по времени
создания или использовать общий список под видом исходного диалога.
