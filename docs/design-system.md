...
## Состояния загрузки и пустые списки

Первичная загрузка использует `LoadingPlaceholder`: он сохраняет геометрию,
но показывает скелетон только через 200 мс. Данные и ошибки появляются сразу,
без минимального времени показа. Анимация появления отключена при reduced motion.
Повторная загрузка фильтра сохраняет строки только в пределах текущего бизнеса.

Явное действие пользователя немедленно показывает процесс и блокирует повторное
нажатие; задержка скелетона не применяется к кнопкам и индикаторам действий.
Ошибку запроса нельзя показывать как пустой список. Пустое состояние содержит
контекстный CTA: создать запись, открыть чат, подключить канал или сбросить фильтры.
CTA учитывает права пользователя; повтор внешней публикации требует отдельного
намеренного действия, а не универсальной кнопки повтора ошибки.
