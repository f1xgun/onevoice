...
Only contacts registered in the organisation profile supply the allowlist. Ad hoc contacts in messages never grant exemptions, regardless of publication requests, wording, or quotation. The Russian system prompt instructs the assistant to explain that an unregistered phone or e-mail was hidden for privacy and suggest adding it through «Настройки → Организация», without inventing a contact or silently publishing the placeholder. `BusinessContext` currently carries phone and website but no organisation e-mail; this change does not extend the chat API contract.
