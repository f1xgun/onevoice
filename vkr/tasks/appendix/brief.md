...
**Roadmap task:** Оформить приложения: ТЗ, листинги кода, диаграммы, экранные формы

### Назначение
- Подготовить материалы для раздела `appendix` в формате, удобном для фактчекинга и последующей сборки в `vkr/draft/draft.md`.
