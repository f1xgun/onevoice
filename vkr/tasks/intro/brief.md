...
**Roadmap task:** Написать ВВЕДЕНИЕ (3-4 страницы): актуальность, цель, задачи, объект/предмет, методы, новизна

### Назначение
- Подготовить материалы для раздела `intro` в формате, удобном для фактчекинга и последующей сборки в `vkr/draft/draft.md`.
