...
.PHONY: lint lint-rbac lint-urls lint-frontend lint-migrations lint-compose-env lint-all fmt fmt-fix docs-check check-llm-defaults
...
.PHONY: clean certs mtls-certs mtls-check clean-certs nats-creds clean-nats-creds
...
lint-all: lint lint-rbac lint-urls lint-frontend lint-migrations lint-compose-env check-legal-versions-parity check-llm-defaults docs-check lint-no-pprof ## Run all linters (Go + RBAC drift + URL check + frontend + migration parity + compose env + legal version parity + LLM defaults + docs)
...
# NATS creds: per-service nkey seeds + the server authorization block (subject
# ACLs), generated by tools/natskeygen. Idempotent: running again with an
# existing infra/nats/creds/auth.conf is a no-op. Run before `docker compose up`.
nats-creds: ## Generate per-service NATS nkey seeds + auth.conf into infra/nats/creds/ (idempotent)
	@go run ./tools/natskeygen

clean-nats-creds: ## Remove all generated NATS creds (next nats-creds rebuilds)
	@find infra/nats/creds -mindepth 1 ! -name '.gitignore' -delete 2>/dev/null || true
	@echo "infra/nats/creds/ cleaned (kept .gitignore)"
