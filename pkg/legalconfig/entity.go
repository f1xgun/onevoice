...
// and the current policy versions (D-19, D-20). Imported by services/api
...
// API startup (D-19). When ANY field is empty or still placeholder, the
// renderer falls back to the «[Юридическое лицо — будет обновлено]»
// stub and the launch-readiness checklist (D-29) flags the deployment.
type Entity struct {
...
// — matches the frontend FE copy in 22-UI-SPEC (D-22).
...
// placeholder / empty state. The pre-launch checklist (D-29) runs this
