...
func TestChatProxy_EnrichesContext(t *testing.T) {
	...
	h := NewChatProxyHandler(mockBiz, mockInteg, orchServer.URL, nil)
...
func TestChatProxy_NoBusiness(t *testing.T) {
	...
	h := NewChatProxyHandler(mockBiz, mockInteg, "http://unused", nil)
...
func TestChatProxy_OrchestratorDown(t *testing.T) {
	...
	h := NewChatProxyHandler(mockBiz, mockInteg, "http://127.0.0.1:1", nil)
