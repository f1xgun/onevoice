...
export function RegisterForm({ inviteOnly = false }: RegisterFormProps) {
  ...
  const onSubmit = async (data: RegisterInput) => {
    ...
    try {
      ...
      if (status === HTTP_STATUS.FORBIDDEN && code === 'registration_invite_required') {
        setFormError(tRegErrors('inviteRequired'));
        setShowWaitlistLink(true);
        return;
      }
      if (status === HTTP_STATUS.SERVICE_UNAVAILABLE && code === 'registration_gate_unavailable') {
        setFormError(tRegErrors('gateUnavailable'));
        return;
      }
      ...
      if (status === HTTP_STATUS.CONFLICT) {
        setError('email', { type: 'server', message: tReg('emailExists') }, { shouldFocus: true });
        return;
      }
  ...
  return (
    <AuthShell
      ...
      title={tReg('title')}
      description={tReg(inviteOnly ? 'closedBeta.invitedDescription' : 'description')}
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          />
          {errors.name && (
            <p id="name-error" role="alert" className="text-sm text-[var(--ov-danger)]">
              {errors.name.message}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          />
          {errors.email && (
            <p id="email-error" role="alert" className="text-sm text-[var(--ov-danger)]">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {errors.confirmPassword && (
              <p
                ...
                role="alert"
                className="text-sm text-[var(--ov-danger)]"
              >
