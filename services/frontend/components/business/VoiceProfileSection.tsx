...
export function VoiceProfileSection() {
  ...
  if (readPerm.isError) {
    return <PermissionLoadError onRetry={readPerm.refetch} />;
  }

  if (!activeBusinessId || readPerm.isLoading || !readPerm.allowed) {
    return null;
  }

  if (profileQuery.isError) {
    return <p className="text-xs text-[var(--ov-danger)]">{t('loadError')}</p>;
  }
    ...
    <form className="flex flex-col gap-4" onSubmit={handleSave}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
        ...
        <Button
          ...
          variant="primary"
          size="md"
          disabled={!form.formState.isDirty || overCap || mutation.isPending || !canEdit}
        >
