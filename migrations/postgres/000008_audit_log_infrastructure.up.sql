-- Phase 19: Audit log infrastructure
...
-- 1. Schema changes -------------------------------------------------------
-- Add business_id (nullable per D-01: failed-login events have no business).
ALTER TABLE audit_logs
    ADD COLUMN business_id UUID NULL REFERENCES businesses(id) ON DELETE CASCADE;

-- Drop NOT NULL on user_id (D-04b / D-31: failed-login writes NULL user_id).
ALTER TABLE audit_logs
    ALTER COLUMN user_id DROP NOT NULL;

-- Note: details column is already JSONB in this path (init.up.sql:82); no cast needed (D-02).

-- Composite index for per-business listing (D-03).
